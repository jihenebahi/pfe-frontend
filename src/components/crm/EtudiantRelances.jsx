// src/components/crm/EtudiantRelances.jsx
...
export default function EtudiantRelances({ etudiantId, etudiantNom, formations_ids = [], onHistoriqueUpdated }) {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getEtudiantRelances(etudiantId);
      setRelances(data);
    } catch (err) {
  ...
  const handleCreate = async ({ dateRelance, commentaire }) => {
    try {
      ...
      const created = await createEtudiantRelance(etudiantId, { 
        dateRelance, 
        commentaire,
        formationId: formations_ids[0] || null,
      });
  ...
  const confirmDelete = async () => {
    try {
      await deleteEtudiantRelance(deleteConfirmId);
      setRelances((prev) => prev.filter((r) => r.id !== deleteConfirmId));
      if (onHistoriqueUpdated) onHistoriqueUpdated();
    } catch (err) {
  ...
  const confirmOk = async () => {
    ...
    try {
      ...
      const { relance: updated } = await actionOkEtudiant(pendingOkId, okNotes);
          ...
          <div className="relance-modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relance-modal-body">
              <p className="ok-prompt-info">
                ...
                L'appel sera enregistré dans l'historique de <strong>{etudiantNom}</strong>.
              </p>
